...
def compare_groups_statistically(
    ...
    def _jonckheere_trend(samples_dict: dict, group_order: list, alpha: float = 0.05):
        ...
        try:
            # scipy >= 1.7.0 공식 함수 시도
            res = scipy_stats.jonckheere_terpstra(ordered, alternative="greater")
            J_stat, p_val = float(res.statistic), float(res.pvalue)
        except AttributeError:
            # 직접 구현: JT 통계량 = 모든 순서쌍 (i<j) 에서 Mann-Whitney U 합
            J_stat = 0.0
            ...
            if Var_J <= 0:
                return {"skipped": "분산 계산 불가"}
            z = (J_stat - E_J) / Var_J ** 0.5
            p_val = float(scipy_stats.norm.sf(z))  # one-sided (greater)
        return {
            "J":           round(J_stat, 4),
            "p":           round(p_val,  6),
            ...
            "conclusion":  "단조 증가 트렌드 유의" if p_val < alpha else "트렌드 없음",
        }
